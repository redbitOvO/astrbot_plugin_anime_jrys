...
- ZHUQIY: `https://r.zhuqiy.com/en/`，通用 PC 横图兜底源，可能混入现实/通用壁纸，默认关闭
...
每次生成图片时，插件对每个已启用关键词源只尝试一个随机关键词/标签。关键词源按 Wallhaven、Konachan、Safebooru、Danbooru 的顺序尝试；关键词源全部失败后，才进入 Waifu.im 兜底源。ZHUQIY 属于通用 PC 横图兜底源，可能混入现实/通用壁纸，默认关闭；只有手动开启 `enable_zhuqiy_fallback` 时才会参与兜底。
...
- `enable_zhuqiy_fallback`: 关键词源失败后是否启用 ZHUQIY 通用横图兜底，默认关闭。该源不保证纯动漫图，可能返回现实壁纸。
- `enable_waifu_im_fallback`: 关键词源失败后是否启用 Waifu.im 动漫兜底。
...
欢迎进入今日商店！
金币余额：42
以下是可用商品：商品1：2金币——重新测运；商品2：5金币——随机图片；商品3：8金币——随机涩图。请输入商品编号进行购买！
```
...
- 商品 2：花费 `5` 金币获得一张普通 SFW 随机动漫原图，图源由 `shop_sfw_sources` 单独控制。
- 商品 3：花费 `8` 金币获得一张 18+ 随机原图，图源由 `shop_nsfw_sources` 单独控制，默认使用 Yande.re / Danbooru / Gelbooru 高分图源。
...
- `shop_sfw_sources`: 商品 2 普通随机图片图源顺序，默认 `wallhaven;safebooru;waifu_im`。可用值：`wallhaven`、`konachan`、`safebooru`、`danbooru`、`waifu_im`、`zhuqiy`。
- `enable_nsfw_shop`: 是否启用 18+ 商品，默认开启。
- `shop_nsfw_sources`: 商品 3 的 18+ 图源顺序，默认 `yandere;danbooru;gelbooru`。可用值：`yandere`、`danbooru`、`gelbooru`。Danbooru / Gelbooru 在部分服务器上可能需要登录或 API 权限，Yande.re 通常更适合作为无鉴权默认源。旧配置名 `nsfw_sources` 仍会兼容读取。
...
- `Danbooru 403 Just a moment...`: 通常是 Danbooru / Cloudflare 对服务器 IP 的挑战页拦截。API Key 只能解决应用层鉴权，不能绕过 Cloudflare 挑战；建议保持 `enable_danbooru_source = false`，或继续依赖 Safebooru / Waifu.im。

只要 Waifu.im 兜底源可用，插件仍会正常出图。如手动启用 ZHUQIY，可能混入非动漫通用壁纸。
